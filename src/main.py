...
async def waitForPayment(
    ctx: InteractionContext, uuid
) -> bool | XummGetPayloadResponse:
    status = xummInstance.checkStatus(uuid)
    maxWait = 120
    currentWait = 0
    try:
        ...
        embed = Embed(title="Transaction Failed", timestamp=datetime.now())
        if str(e) == "PaymentTimeout":
            embed.description = (
                f"Transaction timeout, please complete the process within {maxWait}s"
            )
...
@slash_command(
    name="battle-verify", description="Verify if you are ready for the battle"
)
...
# Choose NFT Command:
@slash_command(
    name="choose-nft", description="Choose which NFT you want to use for the battle"
)
async def chooseNft(ctx: InteractionContext):
    await ctx.defer(
        ephemeral=True, suppress_error=True
    )  # Defer the response to wait for the function to run.
    (
    ...
    try:
        ...
        if str(e) == "DiscordIdNotFound":
            await ctx.send(
                "XRP Wallet not found, use /battle-verify to verify your XRP Wallet"
            )
            return
    ...
    latestMessage = await ctx.send(
        "Select NFT", components=[selectOne, selectTwo], ephemeral=True
    )

    previous_button = Button(
        style=ButtonStyle.GREEN, label="Previous", custom_id="previous", disabled=True
    )
    next_button = Button(
        style=ButtonStyle.GREEN, label="Next", custom_id="next", disabled=True
    )
    ...
    async def wait_component(components, ctx: InteractionContext) -> Component | None:
        try:
            component_result: Component = await client.wait_for_component(
                components=components, check=check, timeout=900
            )
    ...
    def update_group_menu_options(nftOptions, chosen_group, page):
        ...
        group_options = [
            StringSelectOption(
                label=item["label"], value=f"{start+index},{chosen_group}"
            )
            for index, item in enumerate(nftOptions[chosen_group][start:end])
        ]
        return group_options

    component_result: Component = await wait_component(
        [nftMenu, groupMenu, pagination_row], ctx
    )

    while component_result:
        if component_result.ctx.custom_id == "groupSelection":
            chosen_group = component_result.ctx.values[0]
            groupMenu.options = update_group_menu_options(
                nftOptions, chosen_group, page
            )
            ...
            latestMessage = await ctx.edit(
                content=f"Select NFT from {chosen_group} group", components=components
            )
            ...
            chosen_nft = nftOptions[chosen_group][
                int(component_result.ctx.values[0].split(",")[0])
            ]
            ...
            (
                loggingInstance.info(f"NFT Choice: {chosen_nft['label']}")
                if botVerbosity
                else None
            )
            return
            ...
            next_button.disabled = (page + 1) * items_per_page >= len(
                nftOptions[chosen_group]
            )
        ...
        component_result = await wait_component(
            [nftMenu, groupMenu, pagination_row], ctx
        )
...
async def fillXrainReserves(ctx: InteractionContext):
    ...
    (
        loggingInstance.info(f"/fill-xrain-reserve called by {ctx.author.display_name}")
        if botVerbosity
        else None
    )
    ...
    try:
        ...
        if str(e) == "DiscordIdNotFound":
            await ctx.edit(
                content="XRP Wallet not found, use /battle-verify to verify your XRP Wallet"
            )
            return
    ...
    (
        loggingInstance.info(f"Creating payment request for {fillAmount} XRAINS")
        if botVerbosity
        else None
    )
    ...
    embed = Embed(
        title=f"Refill {fillAmount} XRAIN to your Battle Royale reserves",
    ...
    (
        loggingInstance.info(
            f"/fill-xrain-reserve called of {ctx.author.display_name} success"
        )
        if botVerbosity
        else None
    )
...
@slash_command(
    ...
    options=[
        slash_int_option(
            ...
            name="boost-amount",
            choices=[
                SlashCommandChoice(name="3 Boost = 150 XRAIN", value=3),
...
async def buyBoosts(ctx: InteractionContext):
    ...
    (
        loggingInstance.info(f"/buy-boost called by {ctx.author.display_name}")
        if botVerbosity
        else None
    )
    ...
    (
        loggingInstance.info(f"Creating payment request for {xrainPayment} XRAIN")
        if botVerbosity
        else None
    )
    paymentRequest = xummInstance.createXrainPaymentRequest(
    ...
    embed = Embed(
        title=f"Refill {xrainPayment} XRAIN to your Battle Royale reserves",
...
@slash_command(
    name="nft", description="See the NFT that you are using for the battle royale!"
)
async def getNFT(ctx: InteractionContext):
    await ctx.defer(ephemeral=True, suppress_error=True)

    (
        loggingInstance.info(f"/nft called by {ctx.author.display_name}")
        if botVerbosity
        else None
    )
    ...
    if nftInfo["nftToken"] == "" or nftInfo["nftLink"] == "":
        await ctx.edit(
            content="Battle NFT not found, use /choose-nft to choose your Battle NFT"
        )
        return

    embed = Embed(
        title=f"Battle NFT: {nftInfo['nftGroupName']} ***{nftInfo['nftToken'][-6:]}**",
    ...
    embed.add_field(name="Battle Royale Rank", value=nftInfo["battleRank"], inline=True)
    embed.add_field(name="XRAIN power", value=str(nftInfo["xrainPower"]), inline=True)
    embed.add_field(
    ...
    embed.add_field(
        name="Boosts Remaining", value=str(nftInfo["reserveBoosts"]), inline=True
    )
    embed.add_field(
        name="XRAIN Reserves", value=str(nftInfo["reserveXrain"]), inline=True
    )
    ...
    (
        loggingInstance.info(f"/nft called by {ctx.author.display_name} success")
        if botVerbosity
        else None
    )
...
async def battleRoyale(ctx: InteractionContext):
    await ctx.defer()
    (
        loggingInstance.info(f"/br called by {ctx.author.display_name}")
        if botVerbosity
        else None
    )
    ...
    embed = Embed(
        title="XRPL Rainforest Battle Royale!!",
    ...
    (
        loggingInstance.info(f"{len(playersReactor)} players attempted to join")
        if botVerbosity
        else None
    )
    ...
    async def savePlayers(
        ctx: InteractionContext, users: User = None, wager=0, npc=False
    ):
        try:
            if not npc:
            ...
            match str(e):
                case "xrpIdNotFound":
                    ...
                    (
                        await ctx.channel.send(
                            f"{users.mention}, not found. Please verify your wallet first via /battle-verify"
                        )
                ...
                case "insufficientCredits":
                    (
                        loggingInstance.error(f"{users.id} insufficient credit")
                        if botVerbosity
                        else None
                    )
                    (
                        await ctx.channel.send(
                            f"Insufficient  credits for {users.mention}. Please refill your XRAIN reserves"
                        )
                        if users is not None
                        else None
                    )
                case "BattleNFTNotFound":
                    (
                        loggingInstance.error(f"{users.id} battle nft not found")
                        if botVerbosity
                        else None
                    )
                    (
                        await ctx.channel.send(
                            f"Battle NFT not found for {users.mention}. Please set your NFT via /choose-nft"
                        )
                        if users is not None
                        else None
                    )
        ...
        totalBoost = int(playerInfo["xrainPower"]) * (
            2 if int(playerInfo["reserveBoosts"]) > 0 else 1
        )

        playerInstance = Players(
            xrpId=playerInfo["xrpId"],
            wager=wager,
        ...
        try:
            ...
            loggingInstance.error(
                f"Failed to fetch NFT image for {playerInstance.xrpId}: {str(e)}"
            )
            # Set a default/placeholder image or None
            playerInstance.addNFTImage(
                await fetchImage(
                    dbInstance.get_default_nftLink(playerInstance.nftGroupName)
                )
            )
    ...
    async def randomWait():
        waitTime = max(
            random() * gameConfig.getfloat("max_wait"), gameConfig.getfloat("min_wait")
        )
    ...
    while battleResults["winner"] is None:
        ...
        (
            loggingInstance.info(
                f"[Round {roundNumber}]: {battleResults['participantsNum']}/{len(battleInstance.players)} alive"
            )
            if botVerbosity
            else None
        )
    ...
    winnerImageEmbed = Embed(
        title="XRPLRainforest Battle Royale Winner", color=winnerEmbedColor
    )
    ...
    winnerDescription += (
        f" **__{battleInstance.totalWager} XRAIN__** has been sent to you!!"
        if not battleResults["winner"].npc
        else ""
    )
    winnerTextEmbed = Embed(description=winnerDescription, color=winnerEmbedColor)
    winnerTextEmbed.add_field(
        name="Kills", value=f":knife:{battleResults['winner'].kills}", inline=True
    )
    winnerTextEmbed.add_field(
        name="Revives", value=f":wing:{battleResults['winner'].reviveNum}", inline=True
    )

    claimDescription = await dbInstance.getClaimQuote(battleResults["winner"].taxonId)
    claimEmbed = Embed(
        description=f"**{claimDescription['description']}**", color=winnerEmbedColor
    )

    statsEmbed = Embed(
        title="XRPL Rainforest Battle Royale Stats!",
        timestamp=datetime.now(),
    ...
    await dbInstance.addWin(
        battleResults["winner"].xrpId,
        battleResults["winner"].NFT,
        battleResults["winner"].npc,
    )
    ...
    (
        loggingInstance.info(
            f"Winner: {battleResults['winner'].mention if not battleResults['winner'].npc else "NPC"}"
        )
        if botVerbosity
        else None
    )
    ...
    if not battleResults["winner"].npc:
        (
            loggingInstance.info(
                f"Sending {battleInstance.totalWager} XRAIN to {battleResults['winner'].xrpId}"
            )
            if botVerbosity
            else None
        )
        await xrplInstance.registerSeed(xrplConfig["seed"])
        await xrplInstance.sendCoin(
            address=battleResults["winner"].xrpId,
...
async def preRoundInfo(
    ...
    preRoundEmbed = Embed(
        title=f"ROUND {roundNumber}", description=descriptionText, color=roundColor
    )
...
async def postRoundInfo(channel: InteractionContext.channel, battleResults, roundColor):
    ...
    postRoundEmbed.add_field(
        name="Participants", value=battleResults["participantsNum"], inline=True
    )
...
async def fetchImage(url):
    try:
        ...
        if response.status_code != 200:
            loggingInstance.error(
                f"fetchImage: HTTP error {response.status_code} for URL={url[:100]}"
            )
            raise Exception(f"HTTP {response.status_code} error fetching image")

        if not response.content:
            loggingInstance.error(
                f"fetchImage: Empty response content from URL={url[:100]}"
            )
            raise Exception("Empty image content received")

        loggingInstance.info(
            f"fetchImage: Image fetched successfully, size={len(response.content)} bytes"
        )

        image = Image.open(BytesIO(response.content))
        loggingInstance.info(
            f"fetchImage: Image opened successfully, format={image.format}, size={image.size}"
        )
        ...
        loggingInstance.error(
            f"fetchImage: Timeout fetching image from URL={url[:100]}"
        )
        raise Exception("Image fetch timeout")
    except requests.exceptions.ConnectionError as e:
        loggingInstance.error(
            f"fetchImage: Connection error for URL={url[:100]} - {str(e)}"
        )
        raise Exception("Image fetch connection error")
    except requests.exceptions.RequestException as e:
        loggingInstance.error(
            f"fetchImage: Request exception for URL={url[:100]} - {str(e)}"
        )
        raise Exception(f"Image fetch failed: {str(e)}")
    except Exception as e:
        loggingInstance.error(
            f"fetchImage: Unexpected error for URL={url[:100]} - {str(e)}"
        )
        raise
...
async def create_collage(images):
    ...
    for entry in possibleEntryPerRow:
        ...
        if currentRemainder in remainderResult.keys():
            ...
            remainderResult[currentRemainder] = (
                currentEntry if currentEntry > entry else entry
            )
    ...
    resized_images = [
        img.resize((min_width, min_height), Image.Resampling.LANCZOS) for img in images
    ]
